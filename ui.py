...
def draw_text(surface, text, pos, color, font=FONTS["medium"], pos_mode="top_left",
    ...
    surface.blit(label, label_rect) # matnni chizish
...
def button(surface, pos_y, text=None, click_sound=None):
    ...
    pygame.draw.rect(surface, COLORS["buttons"]["shadow"], (rect.x - 6, rect.y - 6, rect.w, rect.h)) # soya to'rtburchagini chizish
    pygame.draw.rect(surface, color, rect) # to'rtburchakni chizish
    # matnni chizish
    if text is not None:
        draw_text(surface, text, rect.center, COLORS["buttons"]["text"], pos_mode="center",
                    shadow=True, shadow_color=COLORS["buttons"]["shadow"])

    if on_button and pygame.mouse.get_pressed()[0]: # agar foydalanuvchi tugmani bossa
        if click_sound is not None: # ovoz o'zgarishi
            click_sound.play()
        return True
